...
class UASocketProtocol(asyncio.Protocol):
    ...
    def _send_request(self, request, timeout: float = 1, message_type=ua.MessageType.SecureMessage) -> asyncio.Future:
        ...
        """
        add_nodes = False
        if request.TypeId.Identifier == 488:
            self.add_node_count += 1
            if self.add_node_count == 1:
                add_nodes = True
            else:
                import pdb; pdb.set_trace()
            if add_nodes:
                self._request_id += 3
                self._connection._sequence_number += 3
            #import pdb; pdb.set_trace()
        ...
        if add_nodes:
            with open("msg.txt", "wb") as f:
                self._connection._sequence_number -= 4
                self._request_id -= 4
                f.write(msg)
                raise ua.UaError("Server didn't respond")
