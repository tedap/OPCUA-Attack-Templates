...
sys.path.insert(0, "/Users/tedap/Documents/Projects/FYP/exp211/open62541/models/vulnerability-models/race-condition/venv/attacker")
...
async def task(loop):
    ...
    url = "opc.tcp://user1:password1@Teds-MacBook-Pro.local:4840/freeopcua/server/"
    ...
    try:
        async with client:
            # User 1 takes control of the session
            await client.activate_session(username="user1", password="password1")
            
            objects = client.nodes.objects
            try:
                # Create the illegal user 1 request
                obj = await objects.add_object(0, "obj702")   
            ...
            # User 2 - Irrelevant request, gets replaced by the proxy
            obj = await objects.add_object(1, "obj2")
