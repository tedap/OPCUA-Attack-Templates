...
class InternalSession(AbstractSession):
    ...
    async def create_session(self, params: ua.CreateSessionParameters, sockname: Optional[Tuple[str, int]] = None):
        ...
        with open("nonce.txt", "r") as f:
            hex_data = f.read()
            self.nonce = bytes.fromhex(hex_data)
        #self.nonce = create_nonce(32)
        result.ServerNonce = self.nonce
        self.logger.info("Server used this nonce to create session 1: %s", self.nonce.hex())
        ...
        if self.iserver.user_manager is not None:
            try:
                if isinstance(id_token, ua.UserNameIdentityToken):
                    ...
                    with open("usertoken.txt", "w") as f:
                        f.write(str(params.UserTokenSignature.Algorithm) + '\n')
                        f.write(params.UserTokenSignature.Signature.hex())
