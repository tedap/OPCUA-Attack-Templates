...
class UserManager:
    ...
    def __init__(self):
        self._users = {}  # store username -> {"password": ..., "user": User}

    async def add_role(self, username: str, password: str, user_role: UserRole):
        if username is None or password is None:
            raise KeyError("Username and password are required")

        if username in self._users:
            logging.warning(
                "User %s already exists, overwriting.", username
            )

        self._users[username] = {"password": password, "user": User(role=user_role, name=username)}

    def add_user(self, username: str, password: str):
        #await self.add_role(username=username, password=password, user_role=UserRole.User)
        self._users[username] = password
    def add_admin(self, username: str, password: str):
        #await self.add_role(username=username, password=password, user_role=UserRole.Admin)
        self._users[username] = password

    def get_user(self, iserver, username=None, password=None, certificate=None):
        """
        Called by the server on login. Return a User object if credentials match.
        """
